...
\begin{figure}[!htb]
	\begin{subfigmatrix}{2}
		\subfigure[Threshold value with respect to the benchmark model (blue) and the capacity optimized model (orange), considering capacity levels $K \in [0, \theta/\alpha=1000)$ and the value that $x^*_B$ takes when considering $K^*_C$ (black).]{\includegraphics[width=0.45\textwidth]{Prob1_CapOpt/xopt_k.pdf}}
		\subfigure[Evaluation of value functions $F$, considering capacities $K=100$ (darker blue) and $K=700$ (lighter blue), and $F^*$ (orange) with respective demand threshold values presented (black)]{\includegraphics[width=0.45\textwidth]{Prob1_CapOpt/F_k100700.pdf}}
	\end{subfigmatrix}
...
Now that the threshold values were analysed, we focus on the optimal capacity $K^*_C$ as it is presented in \eqref{prob1:K*}. As done previously, we will analyze how does $K_C^*$ behaves with the different parameters.
