...
Manipulating \eqref{eq:31} by changing the region of integration of the first integral, solving it and omitting indicator functions - to simplify the notation - we obtain
\begin{align}
...
Once again, the constant term $a_2$ and the threshold value $x_2^*$ are found by value matching \eqref{valuematch} and smooth pasting \eqref{smoothpasting} conditions. Thus, by solving the following system
\begin{equation}
\begin{cases} 
...
\label{3_F1A}
\end{equation}
where coefficient $a_{1,A}$ and the threshold value $x_{1,A}^*$ are found by value matching \eqref{valuematch} and smooth pasting \eqref{smoothpasting} conditions, expressed by the corresponding system
\begin{equation}
...
\begin{figure}[!htb]
	...
	\end{subfigmatrix}
	\caption{Value function $F$ and respective subfunctions associated to different settings of parameter $\eta$.}
	\label{fig:3_F}
\end{figure}
...
\end{align*}
where the denominator is positive since $d_1>1$, $\phi>0$ and the denominator is positive since $r>\mu$ and by noting that $-2 \mu ^2+\mu  \sigma ^2 \left(\phi+1\right)-2 r \sigma ^2<0 \Leftrightarrow \mu d_1-r<0$, which holds always, as previously showed in \eqref{mud1-r}.
