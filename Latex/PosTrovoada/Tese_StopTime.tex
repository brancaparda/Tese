...
On Appendix \ref{chapter:appendixVectors} the most important functions used are presented, concerning both analysis.
...
\node [below of = c1, xshift=2.5cm, yshift=-3cm] (c11) 
{
	...
	$\bullet$ xthreshold: either $x_B^*$ or $x_C^*$, associated to each chapter
};
...
\node [below of = treino, yshift=-2cm] (val) {\textbf{3.}  For each $x_{0j}$,\\
	calculate mean, median and standard deviation of oberved optimal investment times $\tau_{j}$ using obtained values $\{\tau_{ji} \}_{i \in \{ 1,...,1000\} }$};
