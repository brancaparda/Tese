...
a_{1,A}&=
a_2 x^*_2+\frac{((\theta -\alpha  K_1)K_1-2 \eta  K_1 K_0) }{r-\mu }(x_{1,A}^*)^{1-d_1}-\delta K_1 (x_{1,A}^*)^{-d_1}. \label{3:a1A} 
%&= \frac{2^{\text{dd1}} \eta  k \text{k0} \left(\frac{\text{dd1} (1-\alpha  \text{k0}) (r-\mu )}{(\text{dd1}-1) \eta  k r}\right)^{1-\text{dd1}}}{\text{dd1} (r-\mu )}
\end{align}
Note that the threshold $x_{1,A}^*$ needs always to be positive. Therefore the following restriction holds
\begin{equation}
...
Note that from \eqref{3:Fadd} it's impossible to observe a demand threshold $x_2^*$ smaller than $x_{1,A}^*$. This would mean that at the precise instant $x_{1,A}^*$ was reached, we should immediately replace the \textit{old} product by the \textit{new} one. However, this case is not related with thresholds $x_{1,A}$ or $x_2^*$, but with $x_{1,R}^*$.
Therefore we are able to constraint the domains of both functions $F_{1,A}$ and $F_{1,R}$ by analyzing when does $x_{1,A}^*<x^*_2$ happen or not, respectively. That is
\begin{align}
...
\frac{(\theta-\alpha K_1)K_1 x}{r-\mu} -\frac{\pi_0}{r} - \delta K_1   &, \ ( x>x_{1,A}^* \ \wedge \ \eta<\eta^*) \ \overset{.}{\vee} \ (x>x_{1,R}^* \ \wedge \ \eta \geq \eta^*) \\
\end{cases}
	\label{3_madzbigboy}
\end{equation}
...
Each column corresponds a different value of $\eta$ set. In each plot of the first row it's represented the different possible parts associated to the value function $F$ \eqref{3_madzbigboy}, accordingly to the respective legend, being the red dashed line corresponding to the valid value function associated to the parameters considered. It is also represented the three possible thresholds: $x_{1,A}^*$ by the pink dot, $x^*_2$ by the purple dot and $x_{1,R}^*$ by the black dot. Finally on the second row it's represented solely the value function $F$ associated (the same as showed in the respective plot above) and respective threshold values.

\begin{figure}[!htb]
	\begin{subfigmatrix}{6}
		\subfigure[$\eta=0.007$ ]{\includegraphics[width=0.32\textwidth]{Prob3/eta007.pdf}}
		\subfigure[$\eta=0.012$ ]{\includegraphics[width=0.32\textwidth]{Prob3/eta0121.pdf}}
		\subfigure[$\eta=0.013$ ]{\includegraphics[width=0.32\textwidth]{Prob3/eta013.pdf}}
		\subfigure[$\eta=0.007$]{\includegraphics[width=0.32\textwidth]{Prob3/eta007dash.pdf}}
		\subfigure[$\eta=0.012$]{\includegraphics[width=0.32\textwidth]{Prob3/eta0121dash.pdf}}
...
First, note that for this set of values, we obtain $\eta^*\simeq 0.0121121$. Therefore we have, for the plots in the first and second columns, that it's better to invest and have a period of simultaneous production and only after the second threshold being reached, we change to produce solely the \textit{new} product. On the other side, in the third column, we have the situation where, when we invest, we should immediately produce solely the \textit{new} product.

In the first column, we observe that the thresholds to be considered are $x_{1,A}^*$ and $x_2^*$. The first one states upon which demand level we should invest in the \textit{new} product and start the simultaneous production. The second one states upon which demand level we should produce only the \textit{new} product. The respective value function associated to this problem has 3 parts and it's given by 
\begin{equation}
\begin{split}
F(x)=\frac{\pi_0}{r}+
...
\begin{prop}
	\label{3_propx1A}
	Decision threshold $x_{1,A}^*$ increases with $\eta, \ \delta, \ \sigma, \ K_0$ and $K_1$ and decreases with $\theta$ and $\alpha$.
\end{prop}

\textbf{Proof:}

The results regarding parameters $\eta, \ \delta, \ \theta$ and $\alpha$ come immediately by the expression of $x_{1,A}^*$  \eqref{eq:3_x1A}.
...
\end{align*}
where the denominator is positive since $d_1>1$, $\phi>0$ and ?????? and the denominator is positive since $r>\mu$ and by noting that $-2 \mu ^2+\mu  \sigma ^2 \left(\phi+1\right)-2 r \sigma ^2<0 \Leftrightarrow \mu d_1-r<0$, which holds always, as previously showed in \eqref{mud1-r}.
...
\begin{table*}[!htb]
	...
	\begin{tabular}{lllllll}
		...
		$\bullet$ & $\sigma=0.05$ &  & \hspace{7cm} &  &  $\bullet$ & $\theta=10$   \\
		$\bullet$ & $r=0.05$       &  & \hspace{7cm} &  &  $\bullet$ & $\delta=2$        \\
		$\bullet$ & $K_0=90$     &  & \hspace{7cm} &  &  $\bullet$ & $K_1=100$       \\
		$\bullet$ & $\eta=0.007$     &  & 
	\end{tabular}
\end{table*}
we perform some numerical approximations concerning $x_{1,A}^*$. %Note that $\eta$ is chose to be smaller than $\eta^*$.
...
\begin{figure}[!htb]
	...
	\end{subfigmatrix}
	\caption{Threshold value $x^*_{1,A}$ with respect to parameters with which it increases.}
	\label{fig:2_x1Ai}
\end{figure}

On Figure \ref{fig:2_x1Ai} we present some plots that comprove what was stated on Proposition \ref{3_propx1A}. We also add another one, regarding the discount rate $r$, for which we couldn't derive any analytical solution, but studying the behaviour of $x^*_{1,A}$ we conclude that it also increases with $r$.

\begin{figure}[!htb]
	\begin{subfigmatrix}{3}
		\subfigure[$\alpha \in (0, \min \{ 1/K_0, \theta/K_1 \} )$ ]{\includegraphics[width=0.32\textwidth]{Prob3/x1Aalpha.pdf}}
		\subfigure[$\mu \in (0,r)$]{\includegraphics[width=0.32\textwidth]{Prob3/x1Amu.pdf}}
		\subfigure[$\theta \in (1,10)$]{\includegraphics[width=0.32\textwidth]{Prob3/x1Atheta.pdf}}
	\end{subfigmatrix}
...
\begin{figure}[!htb]
	\begin{subfigmatrix}{1}
		\subfigure[$\eta \in (0, \alpha )$ ]{\includegraphics[width=0.45\textwidth]{Prob3/x2sigma.pdf}}
	\end{subfigmatrix}
